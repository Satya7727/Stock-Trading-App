...
app.post("/signup", async (req, res) => {
  try {
    ...
    res.status(201).json({ message: "User created successfully", redirectUrl: "http://localhost:3001/" });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
  }
...
app.post("/login", async (req, res) => {
  try {
    ...
    res.cookie("token", token, { httpOnly: true, secure: false, maxAge: 60 * 60 * 1000 * 24 * 3 });
    res.status(200).json({ message: "Login successful", redirectUrl: "http://localhost:3001/" });
  } catch (err) {
